...
write_page() {
    ...
    title=$2
    page_url=$(echo "$filename" | sed -E 's|.*/(.*).md|/\1|')  # Например, "/graph"

    # Основной контент
    content=$($MARKDOWN "$filename" | sed "$ a <small>$dates_text</small>")

    # Добавляем секцию комментариев только для постов
    case "$filename" in
        posts/*)
            content="$content"'
<style>
...
.isso-comment.isso-is-page-author .isso-avatar img {
    box-shadow: 0 0 12px #ff6b6b;
    border-radius: 50%;
}
</style>
<section id="isso-thread" data-isso-id="'"$page_url"'" data-title="'"$title"'"></section>
<script data-isso="https://comments.sorokin.music" data-isso-page-author-hashes="ff0c0ae5337923403aad2449bccdfc47, 92264c714dff" src="https://comments.sorokin.music/js/embed.min.js" async="async"></script>'
    ...
    # Объединяем с header.html и заменяем {{TITLE}}
    printf '%s\n' "$content" | cat header.html - | sed "s/{{TITLE}}/$title/" > "$target"

    # Gemini-версия (без изменений)
    GEMINI "$filename" | \
        sed "$ s/$/\\n\\n$dates_text/" \
        > "$(echo "$target" | sed s/.html/.gmi/)"
}
